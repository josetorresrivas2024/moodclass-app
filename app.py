...
# Guardar DB en Documentos (para que sea estable en cualquier PC)
DATA_DIR = Path.home() / "Documents" / "MoodClass"
DATA_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = str(DATA_DIR / "moodclass.db")

# PIN docente (cámbialo)
TEACHER_PIN = "1234"
...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS moods (
            ...
            day TEXT NOT NULL,
            moment TEXT NOT NULL,          -- 'entrada' o 'salida'
            is_anonymous INTEGER NOT NULL, -- 0/1
            student_id INTEGER,            -- null si anónimo
...
def emotion_cards(title: str, options, columns: int = 4, state_key: str = "chosen_emotion"):
    ...
    for i, (emoji, label) in enumerate(options):
        with cols[i % columns]:
            bg_color = emotion_colors.get(label, "#FFFFFF")
            ...
            st.markdown(
                ...
                div[data-testid="stButton"] button[key="{state_key}_{i}"] {{
                    ...
                    border: 2px solid {"#111827" if is_selected else "rgba(17,24,39,0.12)"} !important;
                }}
                </style>
                """,
    ...
    if chosen:
        st.markdown(
            ...
            f"<div style='font-size:1.35rem; font-weight:900;'>{chosen}</div></div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
...
.mcard-title {font-weight: 900; font-size: 1.05rem; margin-bottom: 6px;}
.mmuted {color: rgba(17,24,39,.6); font-size: 0.92rem;}
...
.kpi-title {font-size: .85rem; color: rgba(17,24,39,.65); font-weight: 900;}
.kpi-value {font-size: 1.6rem; font-weight: 900; margin-top: 2px;}
.kpi-sub {font-size: .9rem; color: rgba(17,24,39,.65); margin-top: 4px;}
...
.badge-green { background: #E6F7F0; }
.badge-yellow { background: #FFF7D6; }
.badge-red { background: #FFE5E5; }
</style>
""",
    unsafe_allow_html=True,
)
...
st.title("🎒 " + APP_TITLE)
st.caption("Piloto local en tu PC: registro emocional + panel docente + botiquín emocional.")

with st.sidebar:
    st.header("Menú")
    page = st.radio("Ir a:", ["👦 Estudiante", "🧑‍🏫 Docente (Panel)", "⚙️ Configuración (piloto)"])
...
# =========================
if page == "👦 Estudiante":
    st.subheader("Registro emocional (2 minutos)")
    ...
    with col2:
        emotion = emotion_cards("1) Elige tu emoción", EMOTIONS, columns=4, state_key="emo_student")

        st.markdown("### 2) (Opcional) ¿Por qué te sientes así?")
    ...
    st.divider()
    st.info("📌 Tip: Para primaria podemos reducir emociones a 4–5 opciones y botones más grandes.")
...
# =========================
    ...
    st.subheader("Acceso docente (piloto)")
...
# =========================
    ...
    st.subheader("Configuración (piloto)")
    st.info(
